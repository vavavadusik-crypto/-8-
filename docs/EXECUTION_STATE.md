...
PHASE 4 IN_PROGRESS (Project/Client workspace): терминальный claude в ../hermest-board-backend ветка feat/m4-workspace-runtime от 00e83f8. Спека scratchpad/m4-workspace-spec.md: SQLite (node:sqlite — verified zero-dep, без флага) store + миграции (clients/projects/campaigns/content/assets/render+publish jobs/notes/activity-log), CRUD-API под workspace-префиксом, поиск/фильтры, JSON import/export, backup, tenant-boundary + permission-model (single-user default, team-flag), тесты миграций+доступа. Не трогает index.html/src/app.js (frontend-полоса позже мне). NEXT: дождаться → ревью (verify no fake-success) → merge → gate → Gate M4 → PHASE 5 (UX/onboarding).
