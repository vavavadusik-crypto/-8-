...
PHASE 3 IN_PROGRESS (publishing): терминальный claude в worktree ../hermest-board-backend ветка feat/m3-publish-runtime. Спека scratchpad/m3-publish-spec.md: аудит publish-pack/connectors → общий publishing-контракт (draft-default, confirm для live, idempotency, receipt, sanitized) → 1 РАБОЧИЙ безопасно-тестируемый адаптер (generic webhook/export, mock-endpoint) → production-ready config-path + ЧЕСТНЫЙ статус для YouTube/TikTok/Instagram (OAuth PKCE/state, encrypted secrets, platform-validation; needs_oauth_app без креденшелов — NO fake success). Не трогает index.html/src/app.js (моя frontend-полоса позже). NEXT: дождаться → ревью → merge → gate → закрыть Gate M3 → PHASE 4 (Project/Client workspace).
