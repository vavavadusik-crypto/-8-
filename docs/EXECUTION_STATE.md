...
ACTIVE TASK: P2.10 BYOK-UX (ключи через UI → локальный worker, запрос Вадима «чтобы люди сами вставляли API»)
STATUS: IN_PROGRESS
LAST COMMIT: см. git log — P2.5 Ken Burns ЗАКРЫТА: статичные FLUX-фоны получили детерминированный дрейф (4 пресета zoompan по индексу сцены, текст-оверлей резкий, schema-lock на выражения, рантайм доказан keyless-интеграционным тестом с бинарным детерминизмом); гейт 201/201 unit + 5/5 media. ГРАБЛЯ сессии: loudnorm на чистой цифровой тишине (anullsrc) → −∞ LUFS → NaN → падение AAC; синтетика в тестах — только тихий sine
NEXT ACTION: P2.10 — спроектировать: поле ввода ключей (ElevenLabs/FAL/Pexels) в UI → POST на локальный worker → шифрованный BYOK-vault (существующий vault-код) → статус «какие провайдеры executable» в панели рендера; ключ НИКОГДА не попадает в project document/localStorage/manifest. FAL live-smoke — как только Вадим пополнит баланс fal.ai (~$5): команда smoke отработана, код не трогать. ГРАБЛЯ: значения в ~/.secrets/env.sh в ОДИНАРНЫХ кавычках — только `source`, НЕ cut/tr; референс-голоса: George JBFqnCBsd6RMkjVDRZzb · Alice Xb7hH8MSUJpSbSDYk0k2 · Aterna UX4FA7ZvSPh1ma6rI8P9 (кастомный Вадима)
UNCOMMITTED: none
